...
function fit_single_bin(bin_num)
    ...
    cdf_name = fullfile(['cdf_fit_', bin_num, '.png']);
    pdf_name = fullfile(['pdf_fit_', bin_num, '.png']);
    txt_name = fullfile(['para_fit_', bin_num, '.txt']);
    ...
    % --- 定义分布模型 ---
    % 1. Versatile CDF: (1 + exp(-a*(x-c)))^-b
    % p(1)=a, p(2)=b, p(3)=c
    versatile_cdf_func = @(p, x) (1 + exp(-p(1) .* (x - p(3)))).^(-p(2));

    % 2. Versatile PDF: CDF 的导数
    versatile_pdf_func = @(p, x) (p(1) .* p(2) .* exp(-p(1) .* (x - p(3)))) ./ ...
                     ((1 + exp(-p(1) .* (x - p(3)))).^(p(2) + 1));
    % --- 拟合计算 ---
    ...
    % 【关键修复】：处理重复值点
    % 使用 'last' 确保在功率相同时，取对应的最大 CDF 观测值
    [data_unique, ia] = unique(data_sorted, 'last');
    y_unique = y_actual_cdf(ia);

    % 现在执行插值就不会报错了
    actual_cdf_at_eval = interp1(data_unique, y_unique, eval_points, 'linear', 'extrap');
